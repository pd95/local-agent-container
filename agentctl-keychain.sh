...
read_keychain_raw() {
  echo "Reading auth from Keychain ($SERVICE_NAME)" >&2
  if security find-generic-password -a "$ACCOUNT_NAME" -s "$SERVICE_NAME" -w; then return 0; fi
  echo "Failed to read Keychain item for $SERVICE_NAME" >&2
  exit 6
}
...
delete_keychain() {
  echo "Deleting Keychain item ($SERVICE_NAME)" >&2
  if security delete-generic-password -a "$ACCOUNT_NAME" -s "$SERVICE_NAME"; then
    echo "Deleted Keychain item for $SERVICE_NAME"
  else
    echo "Failed to delete Keychain item for $SERVICE_NAME" >&2
    exit 7
  fi
}
...
case "$cmd" in
  ...
  read-raw) [[ $# -eq 1 ]] || usage; read_keychain_raw ;;
  ...
  verify)
    ...
    ;;
  delete) [[ $# -eq 1 ]] || usage; delete_keychain ;;
  *) usage ;;
esac
